...
<body style="background-image: url('../shared/vwsy_trans.png');">
    ...
    <div class="p-4 shadow-lg" style="margin-left: 13vw; min-height: 100vh;" id="productContainer">
        ...
        $sql_result = mysqli_query($conn, "SELECT orders.*, user.username FROM orders JOIN user ON orders.userid = user.userid WHERE orders.uploaded_by = $_SESSION[userid]");
        ...
        while ($row = mysqli_fetch_assoc($sql_result)) {
            ...
            echo '<button class="delete btn-dark p-2 rounded-3 position-absolute end-0 m-4" onclick="altAdd(' . $row['orderid'] . ')" id="btt1' . $row['orderid'] . '">Deliver</button>';
            ...
            echo '<p class="card-text"><small class="text-muted userid">Client UID: ' . $row['userid'] . '<span class="cname" style="margin-left: 10px;">Client Name: ' . $row['username'] . '</span></small></p>';
    ...
    <script>
        ...
        for (const card of productCards) {
            ...
            const userid = card.getElementsByClassName('userid')[0].textContent.toLowerCase();
            const cname = card.getElementsByClassName('cname')[0].textContent.toLowerCase();
        ...
        function altAdd(orderid) {
            res = confirm("Deliver Order?");
            if (res) {
                window.location=`deliver.php?orderid=` + orderid;
            }
        }
